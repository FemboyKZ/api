...
describe("KZ Servers Endpoints", () => {
  ...
  describe("GET /global/servers", () => {
    ...
    it.each([
      ["server name", "?name=Server", "server_name LIKE", ["%Server%"]],
      ["owner steamid", "?owner=76561198000000001", "owner_steamid64 =", []],
      ["approval status", "?approval_status=1", "approval_status =", []],
      [
        "name, sorted A->Z",
        "?sort=name&order=asc",
      ...
      [
        "record count, sorted highest first",
        "?sort=records&order=desc",
        "COALESCE(ss.total_records, 0) DESC",
        [],
      ],
  ...
  describe("GET /global/servers/:id/records", () => {
    ...
    it.each([
      ...
      "filters a server's records by %s",
      async (_name, queryString, fragment) => {
        const { rowsQuery } = await driveList(
          app,
          mockPool,
          `/global/servers/123/records${queryString}`,
          { probes: [[{ id: 1 }]] },
        );
