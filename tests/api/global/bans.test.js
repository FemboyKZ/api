...
describe("KZ Bans Endpoints", () => {
  ...
  describe("GET /global/bans", () => {
    ...
    it.each([
      ["steamid", "?steamid=76561198000000001", "steamid64 ="],
      ["ban_type", "?ban_type=Bhop Hack", "ban_type"],
      ["nothing, sorting newest first", "", "ORDER BY b.created_on DESC"],
    ])("filters by %s", async (_name, queryString, fragment) => {
      const { rowsQuery } = await driveList(
        app,
        mockPool,
        `/global/bans${queryString}`,
      );
