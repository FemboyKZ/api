...
describe("Server Endpoints", () => {
  ...
  describe("GET /servers", () => {
    ...
    // The totals come out of whatever the mock returned, so the filters are
    // checked on the query instead.
    it.each([
      ["game", "?game=csgo", " AND game = ?", ["csgo"]],
      ["status", "?status=1", " AND status = ?", [1]],
    ])("filters by %s", async (_name, queryString, fragment, expected) => {
      pool.query.mockResolvedValueOnce([[]]);

      await request(app).get(`/servers${queryString}`).expect(200);
