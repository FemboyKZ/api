-- Add server details from RCON (hostname, OS, VAC secure status, owner Steam ID, bot count)
ALTER TABLE servers
...
ADD COLUMN steamid VARCHAR(20) DEFAULT NULL COMMENT 'Server owner Steam ID from RCON' AFTER secure,
ADD COLUMN bot_count INT DEFAULT 0 COMMENT 'Number of bots on server' AFTER steamid;
