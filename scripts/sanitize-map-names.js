...
async function sanitizeMapNames(dryRun = false, gameFilter = null) {
  try {
    ...
    // Get all maps
    let query = 'SELECT id, name, game, playtime, server_ip, server_port, last_played, globalInfo, globalInfo_updated_at FROM maps';
    const params = [];
    ...
    changes.slice(0, 10).forEach(change => {
      logger.info(`  [${change.game}] "${change.oldName}" -> "${change.newName}"`);
    });
    ...
    if (dryRun) {
      ...
      const wouldCreateDuplicates = Array.from(duplicates.entries())
      ...
      if (wouldCreateDuplicates.length > 0) {
        ...
        wouldCreateDuplicates.slice(0, 5).forEach(([key, maps]) => {
          ...
          logger.info(`  ${name} (${game}): ${maps.length} entries would be merged`);
          logger.info(`    Total playtime: ${maps.reduce((sum, m) => sum + (m.playtime || 0), 0)} seconds`);
        });
      }
    ...
    for (const [key, mapGroup] of mapGroups.entries()) {
      ...
      if (mapGroup.length === 1) {
        ...
        const totalPlaytime = mapGroup.reduce((sum, m) => sum + (m.playtime || 0), 0);
        
        // Use the most recent last_played
        const lastPlayed = mapGroup.reduce((latest, m) => {
          return !latest || (m.last_played && m.last_played > latest) ? m.last_played : latest;
        }, null);
        
        // Keep globalInfo from the one that has it (prefer primary)
        const globalInfo = mapGroup.find(m => m.globalInfo)?.globalInfo || null;
        const globalInfoUpdatedAt = mapGroup.find(m => m.globalInfo_updated_at)?.globalInfo_updated_at || null;
        ...
        await pool.query(
          `UPDATE maps 
           ...
           WHERE id = ?`,
          [sanitizedName, totalPlaytime, lastPlayed, globalInfo, globalInfoUpdatedAt, primary.id]
        );

        consolidated += duplicatesToMerge.length;
        sanitized++;
        
        logger.info(`Consolidated: ${sanitizedName} (${game}) - merged ${mapGroup.length} entries, total playtime: ${totalPlaytime}s`);
      }
    }
...
if (gameFilter && !['csgo', 'counterstrike2'].includes(gameFilter)) {
  console.error('Invalid game filter. Use --game=csgo or --game=counterstrike2');
  process.exit(1);
}
