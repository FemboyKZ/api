  ...
  try {
    ...
    for (const server of filteredServers) {
      ...
      try {
        ...
        logger.info(`Fetching from Steam Master Server for ${server.ip}:${server.port} with filter: ${filterString}`);
        ...
        logger.error(`Failed to fetch server ${server.ip}:${server.port} from Steam: ${error.message}`);
        // Continue with other servers even if one fails
      }
    }

    logger.info(`Steam Master Server returned ${allResults.length} total servers`);
