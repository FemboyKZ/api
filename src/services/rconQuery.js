...
async function queryRcon(ip, port, password, game) {
  ...
  try {
    ...
    if (isCS2) {
      ...
      logger.debug(`Status: ${statusResponse.length} chars, css_status: ${cssStatusResponse.length} chars`);
    } else {
    ...
    if (error.code === "ECONNREFUSED") {
      logger.debug(`RCON connection refused ${ip}:${port} - check RCON port in config`, errorInfo);
    } else if (error.code === "ETIMEDOUT") {
      logger.debug(`RCON timeout ${ip}:${port} - firewall or server not responding`, errorInfo);
    } else if (error.message?.includes("Authentication failed")) {
      logger.warn(`RCON auth failed ${ip}:${port} - incorrect password`, errorInfo);
    } else {
...
function parseStatusResponse(statusText, isCS2 = false) {
  ...
  const normalizeName = (name) => {
    if (!name) return '';
    return name.replace(/[\x00-\x1F\x7F]/g, '').trim().toLowerCase();
  };
  ...
  for (const line of lines) {
    ...
    if (isCS2 && trimmedLine.match(/^\d+\s+\d+:\d+/)) {
      ...
      const statusMatch = trimmedLine.match(/^\d+\s+(\d+:\d+(?::\d+)?)\s+\d+\s+\d+\s+\w+\s+\d+\s+\S+\s+'([^']+)'/);
      if (statusMatch) {
    ...
    if (isCS2) {
      ...
      if (trimmedLine.includes("Slot") && trimmedLine.includes("Player Name")) continue;
      if (trimmedLine.includes("----") || trimmedLine.includes("#end") || !trimmedLine) continue;
      if (trimmedLine.startsWith("Total players:") || trimmedLine.includes("hostname:") || trimmedLine.includes("os      :")) continue;
      
      // Format: slot playername steamid64 ip ping
      const match = trimmedLine.match(/^(\d+)\s+(.+?)\s+(76561\d{12})\s+(\S+)\s+(\d+)\s*$/);
      ...
      if (trimmedLine.includes("userid") && trimmedLine.includes("name")) continue;
  ...
  logger.debug(`Parsed ${players.length} players (${serverInfo.botCount} bots)`);
...
function convertToSteamID64(steamid) {
  ...
  if (steamID2Match) {
    ...
    const steamId64 = (BigInt(76561197960265728) + BigInt(accountId)).toString();
